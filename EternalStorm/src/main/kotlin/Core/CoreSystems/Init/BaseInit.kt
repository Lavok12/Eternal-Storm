...
object BaseInit {
    ...
    fun loadContent() {
        AppState.coreController.mainContentRegistration.regObjects(AppState.coreController.objectRegistration)
    }

    fun firstFrame(ps: ProcessingSketch) {
        logger.trace("First frame execution")

        AppState.coreController.setScene(GameController(AppState.coreController))
    }
