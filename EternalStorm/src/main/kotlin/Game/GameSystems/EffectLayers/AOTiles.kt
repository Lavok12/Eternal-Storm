...
class AOTiles(var gameRender: GameRender, point: LPoint, mp: Float = 0.7f) : EffectLayer(point, mp) {
    ...
    override fun draw() {
        lg?.noStroke()
        lg?.pg?.clear()
        val mapApi = gameRender.gameController.gameCycle.mapController.mapApi
        val mapSystem = gameRender.gameController.gameCycle.mapController.mapSystem
        ...
        forEachInArea(p1, p2, 1) { ix, iy ->
            ...
            if (mapSystem.containsTile(ix, iy)) {
                tileContext.hp = mapApi.getWallHp(ix, iy)
