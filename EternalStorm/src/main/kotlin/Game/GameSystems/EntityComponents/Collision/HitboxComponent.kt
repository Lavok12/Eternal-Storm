...
class HitboxComponent(var hitboxType: HitboxTypes, entity: Entity, var rigidBody: RigidBody) : EntityComponent(entity), FrameRect {
    var size = 1 v 1
    var delta = 0 v 0
    ...
    private fun checkAxisCollision(horizontalSearch: Boolean) {
        ...
        forEachOnBorder(p1, p2) { x, y, sideX, sideY ->
            val tile = mapApi.getMapTileFromMap(x, y)
            if (tile == null || !tile.isActive) return@forEachOnBorder
    ...
    private fun checkTrigger() {
        ...
        forEachOnBorder(p1, p2) { x, y, _, _ ->
            if (mapApi.getMapTileFromMap(x, y)?.isActive == true) {
                blocksCollision = true
            }
        }
    }
    ...
    override val frameLeftTop: Vec2
        get() = entity.position + delta - size.invertedY() / 2f
