...
class PickUpComponent(
    ...
    private val attractForce = 0.08f
    ...
    fun physicUpdate() {
        ...
        attractDetector.activeContacts.toList().forEach { hitbox ->
            ...
            if (!entity.gameCycle.entityApi.isExist(itemEntity)) return@forEach
            if (itemEntity.item == null) return@forEach

            val dir = (entity.position - itemEntity.position)
            val dist = dir.length()
            if (dist < 0.01f) return@forEach
        ...
        pickUpDetector.activeContacts.toList().forEach { hitbox ->
            ...
            val item = itemEntity.item ?: return@forEach
            if (!container.canAddItem(item)) return@forEach
